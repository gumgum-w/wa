  ...
  (function () {
    function getStore(modules) {
      ...
      let neededObjects = [
        ...
        { id: "AppState", conditions: (module) => (module.STATE && module.STREAM) ? module : null },
...
window.WAPI.getMe = function (done) {
  const raw = window.Store.Contact.get(window.Store.Conn.me);
  const data = {
    id: raw.__x_id,
    phone: raw.__x_id.user,
    displayName: raw.__x_displayName,
    profilePicThumb: raw.__x_profilePicThumb.eurl
  }
...
* @returns {boolean}
*/
window.WAPI.onAppStateChanged = function (callback) {
  // (x,y)=>console.log('statechanged',x,x.state)
  window.Store.AppState.default.on('change:state', callback)
  return true;
}
...
window.WAPI.sendFile = window.WAPI.sendImage;
